...
class SluggableListener extends MappedEventSubscriber
{
    ...
    private $baseTransliterator = array('Gedmo\Sluggable\Util\Urlizer', 'transliterate');

    private $lastTranslisterator = array('Gedmo\Sluggable\Util\Urlizer', 'transliterate');
     ...
     * @return void
     */
    public function setLastTransliterator($callable)
    {
        if (!is_callable($callable)) {
            throw new \Gedmo\Exception\InvalidArgumentException('Invalid transliterator callable parameter given');
        }
        $this->lastTransliterator = $callable;
    }

    public function getBaseTransliterator()
    {
        return $this->baseTransliterator;
    }

    public function getLastTransliterator()
    {
        return $this->lastTransliterator;
    }
    /**
    ...
    public function getOriginalTransliterator()
    {

        return $this->originalTransliterator;
    }
     ...
     */
    private function getHandler($idx, $class)
    {
        if (!isset($this->handlers[$idx. ":" .$class])) {
            $this->handlers[$idx . ":" .$class] = new $class($this);
        }

        return $this->handlers[$idx. ":" .$class];
    }
    ...
    private function generateSlug(SluggableAdapter $ea, $object)
    {
        ...
        foreach ($config['slugs'] as $slugField => $options) {
            ...
            if ($hasHandlers) {
                foreach ($options['handlers'] as $idx => $handlerOptions) {
                    $this->getHandler($idx, $handlerOptions['class'])->onChangeDecision($ea, $options, $object, $slug, $needToChangeSlug, $idx);
                }
            }
            ...
            if ($needToChangeSlug) {
                ...
                if ($hasHandlers) {
                    foreach ($options['handlers'] as $idx => $handlerOptions) {
                        $this->getHandler($idx, $handlerOptions['class'])->postSlugBuild($ea, $options, $object, $slug);
                        if ($this->getHandler($idx, $handlerOptions['class'])->handlesUrlization()) {
                            $urlized = true;
                        }
                    }
                }
                ...
                if ($hasHandlers) {
                    foreach ($options['handlers'] as $idx => $handlerOptions) {
                        $this->getHandler($idx, $handlerOptions['class'])->onSlugCompletion($ea, $options, $object, $slug);
                    }
                }
