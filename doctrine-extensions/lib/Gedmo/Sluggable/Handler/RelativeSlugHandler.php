...
*/
class RelativeSlugHandler implements ChainableSlugHandlerInterface
{
    const SEPARATOR = '/';
     ...
     * {@inheritDoc}
     */
    public function onChangeDecision(SluggableAdapter $ea, array &$config, $object, &$slug, &$needToChangeSlug, $idx)
    {
        $this->om = $ea->getObjectManager();
        $isInsert = $this->om->getUnitOfWork()->isScheduledForInsert($object);
        $this->usedOptions = $config['handlers'][$idx]['options'];
    ...
    {
        $origTransliterator = $this->sluggable->getTransliterator();
        if(!is_object($origTransliterator[0])){
            $this->originalTransliterator = $origTransliterator;
            $this->sluggable->setTransliterator(array($this, 'transliterate'));
            $this->sluggable->setLastTransliterator(array($this, 'transliterate'));
        }
        else{
           $this->originalTransliterator = $this->sluggable->getBaseTransliterator();
           $transliterator = $this->sluggable->getLastTransliterator();
           $transliterator[0]->setOriginalTransliterator(array($this, 'transliterate')); 
           $this->sluggable->setLastTransliterator(array($this, 'transliterate'));
        }
    }

    public function setOriginalTransliterator(callable $originalTransliterator){
        $this->originalTransliterator = $originalTransliterator;
    }
    /**r
     * {@inheritDoc}
     */
        ...
        if ($relation) {
            ...
            if (isset($this->usedOptions['urilize']) && $this->usedOptions['urilize']) {
                $slug = call_user_func_array(
                    $this->sluggable->getBaseTransliterator(),
                    array($slug, $separator, $object)
                );
            }
            if(is_object($this->originalTransliterator[0])){
                $result = $slug.$this->usedOptions['separator'].call_user_func_array($this->originalTransliterator,  array($text, $separator, $object));
            }
            else{
                $result = $slug;
            }
